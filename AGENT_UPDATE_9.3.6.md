...
2. **NOVÝ KROK — Synchronizace base entities do DB** (proveď po načtení svých Drive souborů):
   ...
   a) Najdi spreadsheetId přes search_files (NIKDY neukládat natvrdo — ID se mění po re-upload):
   ```
   search_files(query="{AgentName}_entities", folderId="<entities folder ID>")  → <spreadsheetId>
   sheets_get_values(spreadsheetId=<spreadsheetId>, range="Sheet1")
   ```
   ...
   sync_agent_entities(
     ...
     value_col=1,            -- entities: col 1 = TableName; names/urls: col 0
     meta_cols=["ImportantScore","Description","RowCount","LastPopulated"]  -- ostatní sloupce
   )
   ```
   Parametry `value_col` a `meta_cols` dle entity_type:
   | entity_type | value_col | meta_cols |
   |-------------|-----------|-----------|
   | entity | 1 | ["ImportantScore","Description","RowCount","LastPopulated"] |
   | name | 0 | ["coverage","tables","lastProcessed"] |
   | url | 0 | ["SourceTier","ReliabilityScore","EntityType","LastChecked","Status"] |

   c) Při dalším běhu místo `sheets_get_values` (nebo navíc) použij:
   ```
   get_agent_entities(agent_name="{AgentName}", entity_type="entity")
   ```
