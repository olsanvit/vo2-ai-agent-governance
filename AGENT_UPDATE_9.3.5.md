...
2. Zkontroluj, zda existují tvoje per-agent soubory na Google Drive. Pokud chybí, vytvoř je. Soubory vytváříš bez určení složky (přistanou v root My Drive — operátor je přesune). Cílové složky jsou jen pro orientaci:
   - {AgentName}_entities (Google Sheet) → složka https://drive.google.com/drive/u/0/folders/1-vX64o8hs25FPkAVH52P9lXUhLCC1FjH pro všechny agenty mimo Manager agenty
   - {AgentName}_entities (Google Sheet) → složka https://drive.google.com/drive/u/0/folders/1lEvffJ-rjdExCwMWxmWM7PchnKkGTGUO pro Manager agenty
   - {AgentName}_names (Google Sheet) → složka https://drive.google.com/drive/u/0/folders/1-3GkQT-OqVpkaKwLKkgW8jjbWzNN7BYY
   - {AgentName}_urls (Google Sheet) → složka https://drive.google.com/drive/u/0/folders/1kYViGZR02wNjr1X0PEqJzBQYmjxobm5U
   - {AgentName}_error.txt → složka https://drive.google.com/drive/u/0/folders/1w91GGAKnReBc6bWFtrucjlVMYr7NhURj — vytvoř až při prvním selhání
   Pokud vytvoření souboru selže → zapiš do reportu která chybí, pokračuj.
